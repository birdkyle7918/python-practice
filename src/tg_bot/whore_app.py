        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                if current_count > limit:
                    logger.warning(
                        f"用户 {user_id} 请求 {f.__name__} 接口过于频繁，已达上限。"
                    )
                    return (
                        jsonify(
                            {
                                "code": 429,
                                "message": f"请求过于频繁，请在{per // 3600}小时后重试",
                            }
                        ),
                        429,
                    )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "code": 400,
                    "message": "scheduled_time 格式不正确，应为 YYYY-MM-DD HH:MM:SS",
                }
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "code": 200,
                    "message": f"成功删除 {rows_deleted} 条数据",
                    "row_deleted": rows_deleted,
                }
            ),
            200,
        )
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                row_added = (
                    cursor.rowcount
                )  # rowcount 会返回受影响的行数 (插入成功为1, 忽略为0)
    except mysql.connector.Error as e:
